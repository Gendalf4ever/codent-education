...
async function initializePrinters() {
    ...
    try {
        ...
        await loadMaterialsFromFirebase();
        displayPrintersProducts();
        displayMaterialsProducts();
    } catch (error) {
...
// Загрузка материалов для 3D печати из Firebase
async function loadMaterialsFromFirebase() {
    console.log('=== ЗАГРУЗКА МАТЕРИАЛОВ ДЛЯ 3D ПЕЧАТИ ИЗ FIREBASE ===');
    ...
    try {
        ...
        // Фильтруем только материалы для 3D печати
        materialsData = allProducts.filter(product => {
            const name = (product.name || product.наименование || '').toLowerCase();
            const category = (product.category || product.категория || '').toLowerCase();
            const description = (product.description || product.описание || '').toLowerCase();
            const tags = (product.tags || product.теги || '').toLowerCase();
            
            // Логика фильтрации для материалов 3D печати
            const isMaterial = 
                // Проверяем категорию
                category.includes('материал') ||
                category.includes('material') ||
                ...
                name.includes('фотополимер') ||
                name.includes('photopolymer') ||
                name.includes('материал') ||
                name.includes('material') ||
                // Проверяем описание
                description.includes('материал для 3d') ||
                description.includes('смола для') ||
                description.includes('фотополимер');
            
            if (isMaterial) {
                console.log(`✅ Найден материал для 3D печати: ${name}`);
                console.log(`   Категория: ${category}`);
                console.log(`   Теги: ${tags}`);
            }
        ...
        console.log(`Найдено ${materialsData.length} материалов для 3D печати из ${allProducts.length} общих продуктов`);
        
        if (materialsData.length === 0) {
            console.warn('Не найдено ни одного материала для 3D печати');
        }
...
// Отображение материалов для 3D печати
function displayMaterialsProducts() {
    console.log('=== ОТОБРАЖЕНИЕ МАТЕРИАЛОВ ДЛЯ 3D ПЕЧАТИ ===');
    
    const container = document.getElementById('printing-materials-container');
    if (!container) {
        console.error('Контейнер printing-materials-container не найден');
        return;
    }
    ...
    if (materialsData.length === 0) {
        container.innerHTML = `
            <div class="alert alert-info text-center">
                <i class="fas fa-info-circle fa-2x mb-3"></i>
                <h5>Материалы для 3D печати пока не добавлены</h5>
                <p class="mb-0">Скоро здесь появится каталог материалов для 3D печати</p>
            </div>
        `;
        return;
    }
    ...
    materialsData.forEach(product => {
        const productCard = createMaterialProductCard(product);
        productsHTML += productCard;
    });
    ...
    console.log(`Отображено ${materialsData.length} материалов для 3D печати`);
}

// Создание карточки материала для 3D печати
function createMaterialProductCard(product) {
    ...
    if (imageUrl) {
        // Исправляем проблемные URL изображений
        ...
        if (imageUrl.includes('model-bone.png') || 
            ...
            imageHTML = `<img src="${fixedImageUrl}" class="printer-product-image" alt="${name}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
    ...
    return `
        <div class="col-md-6 col-lg-4 mb-4">
            <div class="printer-product-card">
                ...
                ` : ''}
                <button class="btn btn-outline-primary printer-details-btn" onclick="showMaterialProductDetails('${product.id}')">
...
// Показать детали материала для 3D печати
function showMaterialProductDetails(productId) {
    const product = materialsData.find(p => p.id === productId);
    if (!product) {
    ...
    const modalHTML = `
        <div class="modal fade" id="materialProductModal" tabindex="-1" aria-hidden="true">
    ...
    const existingModal = document.getElementById('materialProductModal');
    if (existingModal) {
    ...
    const modal = new bootstrap.Modal(document.getElementById('materialProductModal'));
    modal.show();
}
